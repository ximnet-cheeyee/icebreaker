...
export function PlayerScreen() {
  ...
  useEffect(() => {
    ...
    if (myGameState.status === 'TARGET_FOUND') {
      ...
      myGameState.status === 'TIMEOUT' ||
      myGameState.status === 'MOVE_DEPLETED'
    ) {
      vibrate(200)
    }
  }, [myGameState])
...
useEffect(() => {
  ...
  // --- early returns below this point ---
      ...
      <Centered>
        <div className="text-center">
          ...
          <p className="text-white/50 mt-2">
            {me.team_id
              ? `You're on ${me.team_id}`
              : "You're in — waiting for the host to set up teams"}
          </p>

          <p className="text-white/30 mt-6 text-sm animate-pulse">
    ...
    return (
      <Centered>
        <div className="text-center px-6">
          ...
          <p className="text-white/50 mt-2">
            Hang tight — you'll join the next match.
          </p>
        </div>
      </Centered>
    )
  ...
  if (!match) {
    return <Centered>Match ended.</Centered>
  }
  ...
  const isSaboteur =
    match.mode === 'HIDDEN_SABOTEUR' &&
    match.saboteur_player_id === me.id

  const isBoardMover =
    match.mode === 'BLIND_NAVIGATOR' &&
    myGameState.board_mover_id === me.id

  const currentPlayerId =
    myGameState.player_order[
      myGameState.current_player_index
    ]
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-purple-950/40 via-slate-950 to-black text-white flex flex-col items-center justify-center gap-6 p-6">
        ...
        <GlitchBanner
          ...
          enabled={
            match.config.reverseGlitchEnabled
          }
        />
        ...
        {isSaboteur && (
          <div className="mt-8 opacity-90">
            <p className="text-center text-xs uppercase tracking-widest text-purple-300/70 mb-2">
              Secret Map
            </p>
      ...
      {finished ? (
        ...
        <>
          ...
          <p className="text-white/60">
            Tell your Board-Mover where to move.
          </p>
        </>
      ) : (
        <div key={currentPlayerId} className="animate-fade-in-fast text-center">
